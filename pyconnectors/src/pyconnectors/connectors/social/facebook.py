...
class FacebookConnector(BaseConnector):
    ...
    def execute(
        ...
        if not access_token:
            raise ValueError("FacebookConnector requires 'access_token' in configuration.")

        api_version = self.config.params.get("api_version", "v19.0")
        base_url = f"https://graph.facebook.com/{api_version}"
        url = f"{base_url}/{endpoint.lstrip('/')}"

        # Append access token for GET requests, or put it in payload
        if method.upper() == "GET":
