...
@connector("storage.ovh")
class OVHConnector(BaseConnector):
    """OVHcloud High Performance Object Storage Connector using boto3 (S3 API)."""
    ...
    def execute(
        ...
        if boto3 is None:
            raise ImportError(
                "OVH connector requires boto3. Install with: pip install pyconnectors[s3]"
            )

        region = self.config.params.get("region_name", "sbg")
        # OVH Object Storage uses this format
        endpoint_url = f"https://s3.{region}.perf.cloud.ovh.net"
